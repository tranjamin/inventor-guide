...
\begin{where-to-find}
All 2D tools are under the \appcommand{2D Sketch Ribbon}. This can be accessed by selecting the \appcommand{Sketch} tab or by creating a new 2D sketch.
\end{where-to-find}
...
\item The \hyperref[subsection: Automatic Dimensions]{\appcommand{Automatic Dimension}} tool will automatically create all the dimensions you could need.
...
\item If you want to recreate a physical object, you can \hyperref[section: Sketch Insert]{\appcommand{Import Picture}} into your sketch and use your geometry tools (most commonly \appcommand{Splines - Interpolation Points}) to draw over it.
\item \hyperref[subsection: Projecting Geometry]{\appcommand{Project Geometry} and \appcommand{Project Cut Edges}} are very powerful tools that lets you copy existing sketches and features into your sketch.
...
\item Inventor allows you to analyse the geometric properties of the sections of your sketches through \hyperref[section: More Sketch Options]{\appcommand{Region Properties}}. You can therefore use Inventor to purely determine the area or moments of inertia of any shape.
\end{itemize}
\end{summary}

\section{Creating a Sketch}

2D sketches can be created in any plane or flat surface. There are many different ways to create a sketch, but the easiest by selecting \appcommand{3D Model \then Sketch \then Start 2D Sketch} and then clicking on the desired plane or flat surface.
\section{Adding Geometry}

\begin{where-to-find}
    Under the \appcommand{Create Panel} of the \appcommand{2D Sketch Ribbon}.
    \end{where-to-find}
...
\begin{itemize}
\item \appcommand{Line}: Creates either simple lines or joined segments of lines and arcs. Click on your starting point, and then your endpoint (to create a line) or click and drag your starting point (to create a curve). Finish the command by double clicking on the final endpoint or clicking \hotkey{ESC}.
\item \appcommand{Spline}: Creates a spline (parametric curve), either by specifying points for the spline to pass through or control vertices. 
\item \appcommand{Equation Curve}: Creates a curve based on a mathematical equation. The equation can be explicit or parametric, and use polar or cartesian coordinates.
\item \appcommand{Bridge Curve}: Creates a curve that joins two line/curve endpoints together such that the bridge curve is tangent to both segments (ref. \hyperref[subsection: Types of Constraints]{Smooth Constraints}).
\item \appcommand{Circle}: Creates a circle, using either a center point or three tangent lines.
\item \appcommand{Ellipse}: Creates an ellipse based on two axial lines
\item \appcommand{Arc}: Creates an arc, using the endpoints and either a center point or midpoint (Three Point Arc). Can also create an arc that joins a line's endpoint and is tangent to it.
\item \appcommand{Rectangle}: Creates a rectangle using various key points.
\item \appcommand{Slot}: Creates a slot or capsule, using various key points.
\item \appcommand{Polygon}: Creates a polygon of any order, and can be drawn inscribed or circumscribed.
\item \appcommand{Fillets} and \appcommand{Chamfers}: Transforms sharp corners into curved corners (fillets) or bevelled corners (chamfers). The radius or angle of these fillets/chamfers can be adjusted in the popup dialog.
\item \appcommand{Text}: Creates text as geometry, which can be further extruded, swept, etc. Various formatting options can be adjusted in the popup dialog.
\item \appcommand{Geometry Text}: Creates text that lies along lines or simple curves. Cannot be used for more complex geometries such as splines.
\item \appcommand{Point}: Creates points.
...
\begin{itemize}
    \item You can usually specify the coordinates of key points or the dimensions of your geometry (magnitude and angle). Press \hotkey{Tab} to specify coordinates or switch between multiple dimensions. You can change the coordinate type (for example, absolute vs relative) by \appcommand{Right Click \then Coordinate Type}.
    ...
    \item You can "snap" your geometry to existing points. A green dot will signify you are placing your geometry on an existing point, and a yellow dot will signify you are placing your geometry along an existing curve. This is often useful to make sure that all your lines connect to form a closed shape. If you are having trouble snapping to geometry, you can \appcommand{Right Click \then Point Snaps} when placing down geometry and choose whether you want to snap to a midpoint, endpoint or various other points.
    \item The above is a case of constraint inference (\hyperref[section: Constraints and Dimensions]{ref. Constraints}), and you can align your geometry in other ways, such as drawing a line which is tangent to another curve. You can temporarily disable constraint inference or point snapping by holding \hotkey{CTRL}.
    \item When starting the \appcommand{Line} command, you can easily create tangent or normal lines. Click your starting point on existing geometry, and then click and drag the starting point. The direction you drag determines whether the line will be tangential or normal.
...
The \appcommand{Project Geometry} feature is a very powerful tool which allows you to copy existing geometry, whether 2D or 3D, to your current sketch. Try to always project geometry which is above the current sketch in the model browser. Generally, projected geometry is adaptive, which means changes to the reference geometry will be reflected in your sketch. To prevent this, hold down \hotkey{CTRL} when projecting geometry, \hotkey{Right Click} on the geometry and select \appcommand{Break Link}, or manually delete the Project Geometry Constraint (\hyperref[section: Constraints and Dimensions]{ref. Constraints}). There are the following options:

\begin{itemize}
\item \appcommand{Project Geometry}: Projects 2D sketches, 3D sketches, object edges and more into your sketch.
\item \appcommand{Project Flat Pattern}: Used predominately in sheet metal projects, unfolds an object into the plane.
\item \appcommand{Project to 3D Sketch}: Used to project your 2D sketch onto a 3D surface. For more information on this group of features, see \hyperref[chapter: 3D Sketches]{3D Sketches} or \hyperref[section: Surfaces]{Surfaces}.
\item \appcommand{Project DWG Geometry}:  Allows you to copy a DWG (drawing) file from your workspace into a sketch. Not very useful for most scenarios.
\end{itemize}
...
\begin{where-to-find}
    Under the \appcommand{Pattern Panel} of the \appcommand{2D Sketch Ribbon}.
    \end{where-to-find}
...
\begin{where-to-find}
    Under the \appcommand{Modify Panel} of the \appcommand{2D Sketch Ribbon}.
    \end{where-to-find}    

The \appcommand{Modify Panel} allows you to make changes to geometry you have already created. You can:

\begin{itemize}
\item \appcommand{Move} geometry.
\item \appcommand{Copy} geometry to a new location.
\item \appcommand{Rotate} geometry.
\item \appcommand{Scale} geometry.
\item \appcommand{Trim} parts of your geometry off by points of intersection.
\item \appcommand{Extend} a line or other geometry until it intersects with another geometry.
\item \appcommand{Split} geometry into multiple parts according to points of intersection.
\item \appcommand{Stretch} certain geometry out.
\item \appcommand{Offset} geometry by creating a slightly larger or smaller copy.
\end{itemize}
...
\item By checking the \appcommand{Precise Input} box on the popup dialog of many of these tools, you can specify the exact coordinates with which you want to modify your geometry with.
...
\begin{where-to-find}
    Under the \appcommand{Constrain Panel} of the \appcommand{2D Sketch Ribbon}.
    \end{where-to-find}
...
Two types of dimensions can be created: regular dimensions and driven dimensions. Driven dimensions are dimensions which are redundant (already specified by existing constraints or dimensions), and thus are only used to display measurements. They are signified in brackets. On the other hand, regular dimensions can be edited to change the geometry. Regular dimensions can be made and unmade driven with \appcommand{Format \then Driven Dimension} or with \appcommand{Right Click \then Driven Dimension}.

Dimensions can be edited by double clicking on the dimension. The formatting of the dimension can be modified by \appcommand{Right Click \then Dimension Properties}, allowing you to specify naming, precision, and tolerance type for both the individual dimension and the default settings.
...
\begin{enumerate}
\item \appcommand{Coincident}: a point is constrained to another point or along another curve.
\item \appcommand{Collinear}: two straight lines (or axes) constrained to lie along the same line.
\item \appcommand{Concentric}: two curves, ellipses or circles constrained to the same center point.
\item \appcommand{Fixed}: locks geometry in place.
\item \appcommand{Parallel}: two lines constrained to lie parallel.
\item \appcommand{Perpendicular}: two lines constrained to lie perpendicular.
\item \appcommand{Horizontal}: a line or pair of points constrained parallel to the x axis.
\item \appcommand{Vertical}: a line or pair of points constrained parallel to the z axis.
\item \appcommand{Tangent}: two curves constrained to be tangent to each other.
\item \appcommand{Smooth}: a splined curve constrained to be tangent to and join endpoints with a curve.
\item \appcommand{Symmetric}: two curves constrained to be symmetric about a third selected line.
\item \appcommand{Equal}: two curves constrained to have the same radius or two lines constrained to have the same length.
\item \appcommand{Project Geometry}: essentially a fixed constraint, it cannot be added to geometry but instead is automatically applied to any projected geometry. If you want to edit projected geometry, you need to remove these constraints.
\end{enumerate}


\subsection{Adding Constraints}
Constraints can be added through the \appcommand{Constrain Panel}. Select the desired constraint in the panel and then select the geometry to be constrained. For some constraints such as the fixed or equal constraint, you can select multiple geometry and then click on the desired constraint to apply to all.
...
To toggle the visibility of a selected geometry's constraints, click on the \appcommand{Show Constraints} icon under the \appcommand{Constrain Panel} in the ribbon.
...
Usually, constraints will restrict you from freemoving geometry. For example, if you have two circles with a concentric constraint, moving one circle will also cause the other to move. When \appcommand{Relax Mode} is enabled, geometry can be moved freely. Instead of also moving constrained geometry, any constraints which are broken will be deleted. In the above case, only the selected circle will move and the concentric constraint will be removed. You can also choose to only enable Relax Mode for specific constraint types.

Under \appcommand{Constraint Settings}, you can:
...
\begin{enumerate}
    ...
    \item Enable \appcommand{Relax Mode}.
\end{enumerate}

You can also edit the scope at which constraints are inferred. Under 

\appcommand{Constraint Inference Scope}, the three options are:

\begin{enumerate}
    \item \appcommand{Geometry in Current Command}: constraints will only be inferred with the same type of geometry (e.g line, arc, rectangle). Remember that guide lines will still appear with reference to other geometry types.
    \item \appcommand{All Geometry}: constraints will be inferred with all geometry, regardless of type or location.
    \item \appcommand{Select}: you can choose which pieces of geometry constraints can be inferred with.

It is also possible to realign a coordinate system, either by relocating the origin or the two axes. Select \appcommand{Edit Coordinate System}, and then relocate either the origin by clicking on a new vertex or the axis/axes by clicking on new edges. To flip the direction of the axes, right click and select the appropriate option. You may need to move the camera in order to do this, and remember that your sketch needs to be below any reference objects in the model browser. 

\subsection{Automatic Dimensions}

\easydifficulty

By selecting \appcommand{Automatic Dimensions and Constraints}, dimensions and \newline constraints can be added to fully paramaterise the sketch. This is a very powerful tool!
...
Geometry can also be custom formatted by changing the pattern, colour and weight. These custom formats can be turned on/off by clicking \appcommand{Format \then Show Format}.
...
\begin{where-to-find}
    Under the \appcommand{Layout Panel} of the \appcommand{2D Sketch Ribbon}.
    \end{where-to-find}
...
\hotkey{Right Click} on the sketch under the part navigator and you can:
\begin{itemize}
...
\mediumdifficulty
While still in your sketch, you can \hotkey{Right Click} on the workspace and:
