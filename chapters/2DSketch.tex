...
2D sketches can be created in any plane or flat surface. There are many different ways to create a sketch, but the easiest by selecting \appcommand{3D Model \textgreater Sketch \textgreater Start 2D Sketch} and then clicking on the desired plane or flat surface.
...
\begin{itemize}
    ...
    \item Existing 3D objects can sometimes obstruct your view of your sketch. You can either change the camera angle or \appcommand{Right Click \textgreater Slice Graphics}.
\end{itemize}

\subsubsection{Types of Geometry}

When selecting the start point of a line, you can click and drag on existing geometry to create a line which is tangent/perpendicular (\hyperref[chap:constraints]{ref. Constraints}). There are several types of geometry which can be placed:
...
Two types of dimensions can be created: regular dimensions and driven dimensions. Driven dimensions are dimensions which are redundant (already specified by existing constraints/dimensions), and thus are only used to display measurements. They are signified by brackets. On the other hand, regular dimensions can be edited to change the geometry. Regular dimensions can be made and unmade driven by selecting \appcommand{Format \textgreater Driven Dimension} or by right clicking and selecting \appcommand{Driven Dimension}.

Dimensions can be edited by double clicking on the dimension. The formatting of the dimension can be modified by right clicking and selecting \appcommand{Dimension Properties}, allowing you to specify naming, precision, and tolerance type for both the individual dimension and the default settings.
...
Settings for constraints can be modified under \appcommand{Constrain \textgreater Constraint Settings} You can:
...
You can also edit the scope at which constraints are inferred. Under \appcommand{Constrain \textgreater Constraint Inference Scope}, the three options are:
...
Geometry can also be custom formatted by changing the pattern, colour and weight. These custom formats can be turned on/off by clicking \appcommand{Format \textgreater Show Format}.
...
By Right Clicking on the sketch under the part navigator, you can:
\begin{itemize}
\item Redefine: if you want to move a sketch from one plane to another. To see the current sketch plane, select Show Input.
...
While still in your sketch, you can right click on the workspace and:

\begin{itemize}
\item Degrees of Freedom: By Right Clicking on the workspace or a piece of geometry, you can display the degrees of freedom present. This is useful to make sure that you have enough dimensions - if you don't, you will see which directions have not been constrained yet.
